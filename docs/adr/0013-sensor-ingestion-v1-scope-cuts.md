...
- **No history/time-series storage.** Only the latest reading per kind is kept (see ADR-0012) — no per-reading history table, matching every other piece of Device telemetry Kuroshiro already stores (`batteryVoltage`, `rssi`, etc., all single overwritten columns).
...
- A future server-attached sensor feature is additive — it needs its own sync mechanism and a `source` discriminator on `DeviceSensor`, but doesn't require reshaping the device-attached path from ADR-0012.
