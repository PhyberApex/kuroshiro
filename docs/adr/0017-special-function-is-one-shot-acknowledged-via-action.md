...
- `Device.specialFunction` keeps its `identify` default (entity column and baseline migration), deliberately. Under one-shot semantics that means a freshly registered Device, and every existing Device whose owner never touched the dropdown, performs one `identify` on its next poll — a Device announcing itself once, which is harmless and arguably useful on first contact. Normalising the default to `none` would cost a migration and an `UPDATE` over existing rows to avoid a cosmetic one-time event; not worth it.
